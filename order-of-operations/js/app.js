...
function toAppUrl(rel) {
  if (!rel) return "";
  try {
    return new URL(rel, APP_BASE).href;
  } catch {
    return rel;
  }
}
...
function render() {
  ...
  // مسار الصورة: يمكن تحديده في JSON (image)، أو يُبنى تلقائيًا حسب id/level
  const imgRel = (q.image === null || q.image === "") ? "" : (q.image || `images/${q.level}/${q.id}.png`);
  ...
  card.innerHTML = `
    ...
    ${imgRel ? `
      <div class="qimgWrap">
        <img class="qimg" src="${toAppUrl(imgRel)}" alt="صورة توضيحية للسؤال: ${escapeAttr(q.title)}" loading="lazy" decoding="async">
      </div>
    ` : ""}
  ...
  // إخفاء الصورة إذا كان المسار غير صحيح أو الملف غير موجود
  const qimg = card.querySelector(".qimg");
  if (qimg) {
    qimg.addEventListener("error", () => {
      const wrap = qimg.closest(".qimgWrap");
