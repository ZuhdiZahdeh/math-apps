...
(() => {
  ...
  function enhanceManualList(question, rootEl) {
    ...
    lis.forEach((li, i) => {
      ...
      // أولوية: ids الصحيحة بحسب الترتيب، ثم خريطة العناوين، ثم مطابقة مباشرة بعنوان النظرية
      const directId = ids[i];
      const id =
        (directId && state.byId.has(directId) ? directId : null) ||
        TITLE_TO_ID[key] ||
        TITLE_TO_ID[raw] ||
  ...
  function renderAutoSection(question, rootEl) {
    ...
    if (!used.length) {
      ...
      if (chips.children.length) {
        ...
        sec.classList.add("theorems-used--empty");
        empty.textContent = "لم يتم العثور على تعريفات النظريات المرتبطة بهذا السؤال داخل ملف النظريات الحالي.";
      }
    }
