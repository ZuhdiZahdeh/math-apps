...
const elementsData = {
    ...
    radius: [
        { type: 'line', p1: 'M', p2: 'A', text: "نصف القطر (م أ): يصل المركز بأي نقطة على المحيط." },
        { type: 'line', p1: 'M', p2: 'B', text: "نصف القطر (م ب): جميع أنصاف الأقطار متساوية." }
    ],
    diameter: [{ type: 'line', p1: 'B', p2: 'C', text: "القطر (ب ج): أطول وتر، ويمر بالمركز." }],
    chord: [
        { type: 'line', p1: 'D', p2: 'E', text: "الوتر (د هـ): يصل بين أي نقطتين على الدائرة." },
        { type: 'line', p1: 'A', p2: 'D', text: "الوتر (أ د): قطعة مستقيمة لا تمر بالمركز." }
    ],
    arc: [{ type: 'arc', s: angles.D, e: angles.E, text: "القوس (د هـ): جزء من محيط الدائرة." }],
    circumference: [{ type: 'circle', text: "المحيط: طول إطار الدائرة المغلق." }]
};
...
function drawIdentify() {
    ...
    idCtx.beginPath(); idCtx.arc(id_cx, id_cy, id_r, 0, 7);
    ...
    Object.values(points).forEach(p => {
        ...
        let tx = p.x + (p.angle ? Math.cos(p.angle) * 30 : 15);
        let ty = p.y + (p.angle ? Math.sin(p.angle) * 30 : -15);
...
function drawExplore() {
    ...
    exCtx.beginPath(); exCtx.arc(cx, cy, r, 0, 7); exCtx.strokeStyle = '#dfe6e9'; exCtx.stroke();
...
function drawQuiz(highlight = null) {
    ...
    qzCtx.beginPath(); qzCtx.arc(225, 225, 150, 0, 7); qzCtx.strokeStyle = '#ecf0f1'; qzCtx.stroke();
    ...
    const qzPoints = {
        M: {x:225, y:225}, A: {x:225 + 150*Math.cos(0.5), y:225 + 150*Math.sin(0.5)},
        B: {x:225-150, y:225}, C: {x:225+150, y:225}, D: {x:225 + 150*Math.cos(3.8), y:225 + 150*Math.sin(3.8)}, E: {x:225 + 150*Math.cos(5.2), y:225 + 150*Math.sin(5.2)}
    };
