...
const LAB_CONFIG = {
    ...
    labVersion: '1.4.0-intro-water-waves',
...
function buildSubmissionPayload() {
    ...
    return {
        ...
        intro: {
            completed: Boolean(labState.intro.completed),
            waterReplayCount: Number(labState.intro.waterReplayCount || 0),
            revealedConcepts: labState.intro.revealedConcepts
        },
...
// ==========================================
// 1.1 التمهيد: موجات الماء واستنتاج مفهوم الدائرة
// ==========================================
...
const conceptCtx = conceptCanvas ? conceptCanvas.getContext('2d') : null;

const introVisualState = {
...
function drawWaterBackground(ctx, width, height) {
    ...
    ctx.strokeStyle = '#8bd3f7';
    ctx.lineWidth = 2;
    for (let y = 40; y < height; y += 38) {
        ctx.beginPath();
        for (let x = 0; x <= width; x += 18) {
            const waveY = y + Math.sin((x + y) / 42) * 6;
...
function drawLabelBox(ctx, text, x, y, options = {}) {
    ctx.save();
    ctx.font = options.font || 'bold 18px Tahoma, Arial';
    ...
    ctx.strokeStyle = options.stroke || '#d6eaf8';
    ctx.lineWidth = 2;
    ctx.beginPath();
...
function drawArrow(ctx, fromX, fromY, toX, toY, color = '#2c3e50') {
...
function drawWaterFrame(timestamp) {
    if (!waterCtx || !waterCanvas) return;

    if (!introVisualState.waterStartedAt) introVisualState.waterStartedAt = timestamp;
    const elapsed = (timestamp - introVisualState.waterStartedAt) / 1000;
    const width = waterCanvas.width;
    const height = waterCanvas.height;
    const cx = width / 2;
    const cy = height / 2 + 10;

    drawWaterBackground(waterCtx, width, height);

    // موجات دائرية متتالية: كلها تنطلق من نقطة سقوط الحجر نفسها.
    for (let i = 0; i < 7; i++) {
        const local = (elapsed - i * 0.72 + 7) % 5.2;
        const radius = 18 + local * 48;
        const alpha = Math.max(0, 1 - local / 5.2);
        if (radius < 255) {
            waterCtx.save();
            waterCtx.globalAlpha = 0.25 + alpha * 0.55;
            waterCtx.strokeStyle = '#2980b9';
            waterCtx.lineWidth = 3;
            waterCtx.beginPath();
            waterCtx.arc(cx, cy, radius, 0, Math.PI * 2);
            waterCtx.stroke();
            waterCtx.restore();
        }
    }

    // مكان الحجر / المركز البصري.
    waterCtx.save();
    waterCtx.fillStyle = '#6b4f3f';
    waterCtx.strokeStyle = '#ffffff';
    waterCtx.lineWidth = 4;
    waterCtx.beginPath();
    waterCtx.arc(cx, cy, 12, 0, Math.PI * 2);
    waterCtx.fill();
    waterCtx.stroke();
    waterCtx.fillStyle = '#e74c3c';
    waterCtx.beginPath();
    waterCtx.arc(cx, cy, 5, 0, Math.PI * 2);
    waterCtx.fill();
    waterCtx.restore();

    drawLabelBox(waterCtx, 'مكان سقوط الحجر', cx + 160, cy - 120, { fill: 'rgba(255, 247, 232, 0.96)', stroke: '#f6c96f' });
    drawArrow(waterCtx, cx + 105, cy - 102, cx + 12, cy - 8, '#8a4b08');

    if (elapsed > 2.2) {
        drawLabelBox(waterCtx, 'الموجات تظهر كدوائر', cx - 150, cy + 135, { fill: 'rgba(232, 244, 253, 0.95)', stroke: '#2980b9' });
    }

    if (elapsed > 4.1) {
        drawLabelBox(waterCtx, 'المركز ثابت', cx, cy + 170, { fill: 'rgba(234, 250, 241, 0.96)', stroke: '#27ae60', color: '#1e8449' });
    }

    const caption = document.getElementById('waterCaption');
    if (caption) {
        if (elapsed < 2.2) {
            caption.textContent = 'راقب النقطة التي سقط فيها الحجر: هل تتحرك أم تبقى ثابتة؟';
        } else if (elapsed < 4.1) {
            caption.textContent = 'لاحظ أن الموجات تنتشر على شكل دوائر متتالية حول مكان الحجر.';
        } else {
            caption.textContent = 'استنتاج أولي: مكان الحجر يمثل مركز الموجات، وكل موجة دائرة حول هذا المركز.';
        }
    }

    introVisualState.waterAnimationId = requestAnimationFrame(drawWaterFrame);
}

function restartWaterAnimation() {
    if (introVisualState.waterAnimationId) {
        cancelAnimationFrame(introVisualState.waterAnimationId);
    }
    introVisualState.waterStartedAt = 0;
    labState.intro.waterReplayCount += 1;
    addLabEvent('intro_water_replay', 'intro', 'شاهد الطالب حركة موجات الماء', {
        replayCount: labState.intro.waterReplayCount
    });
    introVisualState.waterAnimationId = requestAnimationFrame(drawWaterFrame);
}
...
function showConceptPanel() {
    ...
    if (conceptPanel) {
        conceptPanel.classList.remove('hidden-panel');
        conceptPanel.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
    addLabEvent('intro_concept_panel_opened', 'intro', 'انتقل الطالب من الملاحظة إلى استنتاج المفهوم');
    drawConceptScene();
}

function drawConceptScene() {
    if (!conceptCtx || !conceptCanvas) return;
    const width = conceptCanvas.width;
    const height = conceptCanvas.height;
    const cx = width / 2;
    const cy = height / 2;
    const radii = [72, 120, 168];

    conceptCtx.clearRect(0, 0, width, height);
    const gradient = conceptCtx.createLinearGradient(0, 0, 0, height);
    gradient.addColorStop(0, '#f8fbff');
    gradient.addColorStop(1, '#ffffff');
    conceptCtx.fillStyle = gradient;
    conceptCtx.fillRect(0, 0, width, height);

    conceptCtx.save();
    conceptCtx.lineWidth = 4;
    radii.forEach((radius, index) => {
        conceptCtx.strokeStyle = index === radii.length - 1 ? '#2980b9' : '#9bd3f5';
        conceptCtx.beginPath();
        conceptCtx.arc(cx, cy, radius, 0, Math.PI * 2);
        conceptCtx.stroke();
    });
    conceptCtx.restore();
    ...
    conceptCtx.strokeStyle = '#ffffff';
    conceptCtx.lineWidth = 4;
    conceptCtx.beginPath();
    conceptCtx.arc(cx, cy, 9, 0, Math.PI * 2);
    conceptCtx.fill();
    conceptCtx.stroke();
    conceptCtx.restore();

    if (introVisualState.concept.center) {
        drawLabelBox(conceptCtx, 'المركز', cx + 98, cy - 74, { fill: 'rgba(255, 243, 240, 0.97)', stroke: '#e74c3c', color: '#922b21' });
        drawArrow(conceptCtx, cx + 56, cy - 58, cx + 8, cy - 8, '#e74c3c');
    }

    if (introVisualState.concept.radius) {
        const angle = -Math.PI / 7;
        const r = radii[2];
        const px = cx + r * Math.cos(angle);
        const py = cy + r * Math.sin(angle);
        conceptCtx.save();
        conceptCtx.strokeStyle = '#27ae60';
        conceptCtx.lineWidth = 5;
        conceptCtx.lineCap = 'round';
        conceptCtx.beginPath();
        conceptCtx.moveTo(cx, cy);
        conceptCtx.lineTo(px, py);
        conceptCtx.stroke();
        conceptCtx.fillStyle = '#27ae60';
        conceptCtx.beginPath();
        conceptCtx.arc(px, py, 7, 0, Math.PI * 2);
        conceptCtx.fill();
        conceptCtx.restore();
        drawLabelBox(conceptCtx, 'نصف القطر', (cx + px) / 2 + 18, (cy + py) / 2 - 28, { fill: 'rgba(234, 250, 241, 0.97)', stroke: '#27ae60', color: '#1e8449' });
    }

    if (introVisualState.concept.definition) {
        conceptCtx.save();
        conceptCtx.strokeStyle = '#f39c12';
        conceptCtx.lineWidth = 2;
        conceptCtx.setLineDash([8, 8]);
        [-Math.PI / 2, Math.PI / 2, Math.PI].forEach(angle => {
            conceptCtx.beginPath();
            conceptCtx.moveTo(cx, cy);
            conceptCtx.lineTo(cx + radii[2] * Math.cos(angle), cy + radii[2] * Math.sin(angle));
            conceptCtx.stroke();
        });
        conceptCtx.restore();
        drawLabelBox(conceptCtx, 'جميع النقاط على الدائرة تبعد المسافة نفسها عن المركز', cx, height - 42, {
            fill: 'rgba(255, 247, 232, 0.96)',
            stroke: '#f6c96f',
            color: '#8a4b08',
...
function updateConceptSummary() {
    const summary = document.getElementById('conceptSummary');
    if (!summary) return;

    if (introVisualState.concept.definition) {
        summary.textContent = 'الخلاصة: الدائرة هي مجموعة نقاط تبعد المسافة نفسها عن نقطة ثابتة تسمى المركز. هذه المسافة الثابتة تسمى نصف القطر.';
    } else if (introVisualState.concept.radius) {
        summary.textContent = 'نصف القطر هو المسافة أو القطعة المستقيمة من المركز إلى نقطة على الدائرة.';
    } else if (introVisualState.concept.center) {
        summary.textContent = 'مكان سقوط الحجر يشبه مركز الدائرة: نقطة ثابتة تبدأ حولها الموجات.';
    } else {
        summary.textContent = 'اضغط الأزرار بالتدريج ليظهر الاستنتاج أمام الطلاب.';
    }
}

function revealConceptStep(step) {
    if (!introVisualState.concept[step]) {
        introVisualState.concept[step] = true;
        if (!labState.intro.revealedConcepts.includes(step)) {
            labState.intro.revealedConcepts.push(step);
        }
        addLabEvent('intro_concept_revealed', 'intro', `ظهر استنتاج: ${step}`, {
            revealedConcepts: labState.intro.revealedConcepts
        });
    }
    drawConceptScene();
...
function resetIntroDiscovery() {
    introVisualState.concept.center = false;
    introVisualState.concept.radius = false;
    introVisualState.concept.definition = false;
    labState.intro.revealedConcepts = [];
    addLabEvent('intro_concept_reset', 'intro', 'أعاد المعلم/الطالب إخفاء استنتاجات التمهيد');
    drawConceptScene();
    updateConceptSummary();
}

function completeIntroLesson() {
    labState.intro.completed = true;
    addLabEvent('intro_completed', 'intro', 'أكمل الطالب تمهيد موجات الماء واستنتاج المركز ونصف القطر', {
        waterReplayCount: labState.intro.waterReplayCount,
        revealedConcepts: labState.intro.revealedConcepts
    });

    const info = document.getElementById('sessionInfo');
    if (info) {
        info.textContent = 'تم إنهاء التمهيد. الآن يمكنك البدء باستكشاف عناصر الدائرة.';
        info.style.color = '#27ae60';
    }
...
function isLevelUnlocked(tabId) {
    if (tabId === 'intro') return true;
    if (tabId === 'identify') return Boolean(labState.intro && labState.intro.completed);
...
function getLockedReason(tabId) {
    const reasons = {
        identify: 'أكمل التمهيد أولاً: شاهد موجات الماء ثم استنتج المركز ونصف القطر.',
...
function updateNavigationLocks() {
    document.querySelectorAll('.tab-btn[data-tab-id]').forEach(btn => {
        ...
        const completed = tabId === 'intro' ? Boolean(labState.intro && labState.intro.completed) : isLevelQuestionsComplete(tabId);
...
addLabEvent('lab_loaded', 'general', 'تم تحميل درس رحلة اكتشاف الدائرة مع تمهيد موجات الماء ونظام أسئلة موزعة');
