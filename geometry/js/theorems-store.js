import { fetchJson, escapeHtml, shorten, stripHtml, uniq, openImageModal } from "./utils.js";

const DATA_URL = "./data/theorems-master.json";
const state = {
  data: null,
  byId: new Map(),
  aliasToId: new Map(),
...
export async function loadTheorems() {
  ...
  if (!state.loading) {
    state.loading = fetchJson(DATA_URL).then(indexData);
  }
...
export async function openTheoremModal(id, { contextDomain = "plane" } = {}) {
  ...
  modal.bodyEl.innerHTML = `
    ...
    <div class="th-compact-note">هذه البطاقة تُقرأ من ملف <code>data/theorems-master.json</code> نفسه الذي تعتمد عليه صفحة المرجع وصفحات الحلول.</div>
  `;
